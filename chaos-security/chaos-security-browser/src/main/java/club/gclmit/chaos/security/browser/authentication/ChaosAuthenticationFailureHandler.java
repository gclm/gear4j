 ...
 *
 * @author: gclm
 * @date: 2019/12/12 11:26 上午
 * @version: V1.0
 * @since 1.8
 */
@Component("chaosAuthenticationFailureHandler")
public class ChaosAuthenticationFailureHandler extends SimpleUrlAuthenticationFailureHandler {
    ...
    public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response, AuthenticationException exception) throws IOException, ServletException {
        if (LoginResponseType.JSON.equals(securityProperties.getBrowser().getLoginType())) {
            ...
            response.getWriter().write(objectMapper.writeValueAsString(Result.fail(exception.getMessage())));
        } else {
