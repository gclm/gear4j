...
public class AliyunCloudStorageClient extends AbstractStorageClient {
	...
	public AliyunCloudStorageClient(CloudStorage cloudStorage) {
		...
		if (cloudStorage.getType() == StorageServer.ALIYUN) {
			...
			// 创建OSSClient实例
			ossClient = new OSSClientBuilder().build(cloudStorageConfig.getRegion(), cloudStorageConfig.getAccessKeyId(), cloudStorageConfig.getAccessKeySecret());
		} else {
	...
	public void batchDelete(List<String> keys) {
		...
		ossClient.deleteObjects(new DeleteObjectsRequest(cloudStorageConfig.getBucket()).withKeys(keys));
	}
	...
	public void delete(String key) {
		...
		ossClient.deleteObject(cloudStorageConfig.getBucket(), key);
	}
	...
	public FileInfo upload(InputStream inputStream, FileInfo fileInfo) {
		...
		try {
			// 简单上传
			PutObjectResult putObject = ossClient.putObject(cloudStorageConfig.getBucket(), key, inputStream);
		...
		if (key != null) {
			...
			if (StringUtils.isNotBlank(cloudStorageConfig.getEndpoint())) {
				path.append(cloudStorageConfig.getEndpoint());
			} else {
				path.append(cloudStorageConfig.getProtocol()).append("://").append(cloudStorageConfig.getBucket()).append(".").append(cloudStorageConfig.getRegion());
			}
