...
public class UcloudCosClient extends AbstractCosClient implements CosClient {
    ...
    public UcloudCosClient(CosProvider cosProvider) {
        ...
        ossClient = build(cosProvider.getAccessKeyId(), cosProvider.getAccessKeySecret(), cosProvider.getRegion(),
            cosProvider.getEndpoint());
	}
	...
	public FileInfo upload(InputStream inputStream, FileInfo fileInfo) {
		...
		try {
			PutObjectResultBean response = ossClient.putObject(inputStream, inputStream.available(), fileInfo.getContentType())
                ...
                .toBucket(cosProvider.getBucket())
				/*
		...
		if (key != null) {
            ...
            path.append(cosProvider.getProtocol()).append("://").append(cosProvider.getBucket()).append(".")
                .append(cosProvider.getRegion()).append(".").append(cosProvider.getEndpoint()).append("/").append(key);
