...
package club.gclmit.gear4j.cos.service.impl;
...
public class DefaultCosFileServiceImpl extends ServiceImpl<CosFileMapper, FileInfo> implements DefaultCosFileService {
    ...
    public FileInfo uploadFile(MultipartFile file) {
        ...
        if (fileInfo == null) {
            fileInfo = cosClient.upload(file);
    ...
    public FileInfo queryMd5(String md5) {
        ...
        queryWrapper.lambda().eq(FileInfo::getMd5, md5);
    ...
    public FileInfo queryKey(String key) {
        QueryWrapper<FileInfo> queryWrapper = new QueryWrapper<>();
        queryWrapper.lambda().eq(FileInfo::getOssKey, key);
        return cosFileMapper.selectOne(queryWrapper);
    }
    ...
    public List<FileInfo> linkQueryKey(String key) {
        QueryWrapper<FileInfo> queryWrapper = new QueryWrapper<>();
        queryWrapper.lambda().like(FileInfo::getOssKey, key);
    ...
    public List<FileInfo> linkQueryFileName(String fileName) {
        QueryWrapper<FileInfo> queryWrapper = new QueryWrapper<>();
        queryWrapper.lambda().like(FileInfo::getName, fileName);
        return cosFileMapper.selectList(queryWrapper);
    }
    ...
    public List<FileInfo> queryFileSizeBetween(Long startSize, Long endSize) {
        QueryWrapper<FileInfo> queryWrapper = new QueryWrapper<>();
        queryWrapper.lambda().between(FileInfo::getSize, startSize, endSize);
        return cosFileMapper.selectList(queryWrapper);
    }
    ...
    public void deleteFileById(String id) {
        ...
        cosClient.delete(info.getOssKey());
    }
