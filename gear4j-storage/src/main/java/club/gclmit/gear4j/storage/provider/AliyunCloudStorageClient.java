...
import club.gclmit.gear4j.storage.model.contants.FileStatus;
...
import club.gclmit.gear4j.storage.model.properties.StorageProperties;
...
public class AliyunCloudStorageClient extends AbstractStorageClient {
	 ...
	 *
	 */
	public AliyunCloudStorageClient() {
		super(cloudStorage);
		storageProperties.getProviders().
		if (cloudStorage.getType() == StorageServer.ALIYUN) {
			this.cloudStorage = cloudStorage.getConfig();
			log.debug("阿里云配置参数:[{}]", cloudStorage);
			// 创建OSSClient实例
			ossClient = new OSSClientBuilder().build(this.cloudStorage.getRegion(), this.cloudStorage.getAccessKeyId(), this.cloudStorage.getAccessKeySecret());
		} else {
	...
	public FileInfo upload(InputStream inputStream, FileInfo fileInfo) {
		...
		if (key != null) {
            ...
            if (StringUtils.isNotBlank(cloudStorage.getEndpoint())) {
                ...
                path.append(cloudStorage.getProtocol()).append("://").append(cloudStorage.getBucket()).append(".")
                    .append(cloudStorage.getRegion());
			}
