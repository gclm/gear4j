...
package club.gclmit.gear4j.storage.provider;
...
public class UcloudCloudStorageClient extends AbstractStorageClient {
	...
	private static final Logger log = LoggerFactory.getLogger(UcloudCloudStorageClient.class);
	/**
	 * 客户端
	 */
	 ...
	 * OSS 配置参数
	 */
    private final CloudStorage cloudStorageConfig;
	...
	public UcloudCloudStorageClient(CloudStorage cloudStorage) {
		super(cloudStorage);
		log.debug("[Ufile]配置参数:[{}]", cloudStorage);
		if (cloudStorage.getType() == StorageServer.UCLOUD) {
			cloudStorageConfig = cloudStorage.getConfig();
			if (StringUtils.isBlank(cloudStorageConfig.getEndpoint())) {
				cloudStorageConfig.setEndpoint("ufileos.com");
			}
			ossClient = build(cloudStorageConfig.getAccessKeyId(), cloudStorageConfig.getAccessKeySecret(), cloudStorageConfig.getRegion(), cloudStorageConfig.getEndpoint());
		} else {
	...
	public void delete(String key) {
		...
		try {
			ossClient.deleteObject(key, cloudStorageConfig.getBucket()).execute();
	...
	public FileInfo upload(InputStream inputStream, FileInfo fileInfo) {
		...
		try {
			PutObjectResultBean response = ossClient.putObject(inputStream, inputStream.available(), fileInfo.getContentType())
				...
				.toBucket(cloudStorageConfig.getBucket())
				/*
		...
		if (key != null) {
			...
			path.append(cloudStorageConfig.getProtocol()).append("://").append(cloudStorageConfig.getBucket()).append(".").append(cloudStorageConfig.getRegion()).append(".").append(cloudStorageConfig.getEndpoint()).append("/").append(key);
	...
	public ObjectApiBuilder build(String secretId, String secretKey, String region, String endpoint) {
		...
		ExecutorService executorService = ExecutorBuilder.create()
			.setCorePoolSize(5).setMaxPoolSize(50)
