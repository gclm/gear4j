...
public class FastdfsCloudStorageClient extends AbstractStorageClient {
	...
	public FastdfsCloudStorageClient(CloudStorage cloudStorage) {
		...
		if (cloudStorage.getType() == StorageServer.FASTDFS) {
			...
			CloudStorageConfig cloudStorageConfig = cloudStorage.getConfig();
			serverUrl = cloudStorageConfig.getEndpoint() + "/" + cloudStorageConfig.getBucket() + "/";
		} else {
			throw new ChaosException("[{}]:上传文件失败，请检查上传配置", StorageServer.FASTDFS.getName());
		}
	}
