...
package club.gclmit.gear4j.storage.service.impl;
...
import club.gclmit.gear4j.storage.mapper.FileMapper;
import club.gclmit.gear4j.storage.model.pojo.FileInfo;
import club.gclmit.gear4j.storage.provider.CloudStorageClient;
import club.gclmit.gear4j.storage.service.DefaultFileService;
...
public class DefaultFileServiceImpl extends ServiceImpl<FileMapper, FileInfo> implements DefaultFileService {
	...
	public void updateFileStatus(String key, Integer fileStatus) {
		...
		updateWrapper.lambda()
			.eq(FileInfo::getOssKey, key);
		fileMapper.update(new FileInfo(DateUtils.current(), fileStatus), updateWrapper);
	}
	...
	public void updateFileStatusById(String id, Integer fileStatus) {
		FileInfo fileInfo = new FileInfo(Long.valueOf(id), DateUtils.current(), fileStatus);
    ...
    // * @return {@link FileInfo} 文件信息
    // */
    // public FileInfo buildFileInfo(String prefix, MultipartFile file) {
    // String key = getPath(cloudStorage.getConfig().getPrefix(), FileTypeUtils.getSuffix(file));
    // return new FileInfo(file.getOriginalFilename(), file.getContentType(), file.getSize(), SecureUtils.md5(file),
    // key, cloudStorage.getType().getCode());
    // }
}
